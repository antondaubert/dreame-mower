...
class DreameMowerDevice:
    ...
    action_mapping: dict[DreameMowerAction,
                         dict[str, int]] = DreameMowerActionMapping
    ...
    def __init__(
        ...
        self.ai_data: dict[DreameMowerStrAIProperty |
                           DreameMowerAIProperty, Any] = None
        ...
        self._dirty_auto_switch_data: dict[
            DreameMowerAutoSwitchProperty, DirtyData
        ] = {}
        self._dirty_ai_data: dict[
            DreameMowerStrAIProperty | DreameMowerAIProperty, Any
        ] = None
        ...
        self.listen(self._charging_status_changed,
                    DreameMowerProperty.CHARGING_STATUS)
        self.listen(self._cleaning_mode_changed,
                    DreameMowerProperty.CLEANING_MODE)
        self.listen(
            self._ai_obstacle_detection_changed, DreameMowerProperty.AI_DETECTION
        )
        self.listen(
        ...
        self.listen(self._stream_status_changed,
        ...
        self.listen(
            self._off_peak_charging_changed, DreameMowerProperty.OFF_PEAK_CHARGING
        )
        ...
        if self._protocol.cloud:
            ...
            self.listen(self._battery_level_changed,
                        DreameMowerProperty.BATTERY_LEVEL)
            self.listen(
    ...
    def _handle_properties(self, properties) -> bool:
        ...
        for prop in properties:
            ...
            if prop["code"] == 0 and "value" in prop:
                ...
                _LOGGER.debug(
                    "Property %s Not Available", DreameMowerProperty(did).name
                )
        ...
        if not self._ready:
            self.capability.refresh(
                json.loads(
                    zlib.decompress(
                        base64.b64decode(
                            DREAME_MODEL_CAPABILITIES), zlib.MAX_WBITS | 32
                    )
                )
            )
        ...
        if not self._ready:
            ...
            if self.capability.cleaning_route:
                if self.status.cleaning_mode == DreameMowerCleaningMode.MOWING:
    ...
    def _update_status(
        ...
        self._update_property(
            DreameMowerProperty.TASK_STATUS, task_status.value)
            ...
            if map_list and map_list != "":
                try:
                    ...
                    if object_name and object_name != "":
                        _LOGGER.info(
                            "Property MAP_LIST Changed: %s", object_name)
    ...
    def _status_changed(self, previous_status: Any = None) -> None:
        if previous_status is not None:
            ...
            if (
                ...
                status == DreameMowerStatus.CHARGING.value
                and previous_status == DreameMowerStatus.BACK_HOME.value
            ):
                self._cleaning_history_update = time.time()
    ...
    def _charging_status_changed(self, previous_charging_status: Any = None) -> None:
        ...
        if previous_charging_status is not None:
            ...
            if (
                ...
                != DreameMowerChargingStatus.CHARGING_COMPLETED
            ):
                self.schedule_update(2, True)
                ...
                if prop.value in settings:
                    ...
                    if current_value != value:
                        if current_value is not None:
                            ...
                            _LOGGER.info(
                                "AI Property %s Added: %s", prop.name, value)
                        changed = True
            ...
            for prop in DreameMowerAIProperty:
                ...
                if current_value != value:
                    if current_value is not None:
                        ...
                        _LOGGER.info("AI Property %s Added: %s",
                                     prop.name, value)
                    changed = True
                    self.ai_data[prop.name] = value
                        ...
                        if operation:
                            if operation == "monitor" or operation_type == "monitor":
                                self.status.stream_status = (
                                    DreameMowerStreamStatus.VIDEO
                                )
                            elif (
                                operation == "intercom" or operation_type == "intercom"
                            ):
                                self.status.stream_status = (
                                    DreameMowerStreamStatus.AUDIO
                                )
                            elif (
                                ...
                                self.status.stream_status = (
                                    DreameMowerStreamStatus.RECORDING
                                )
    ...
    def _voice_assistant_language_changed(
        ...
        if value and len(value):
            ...
            language_list = {v: k for k,
                             v in VOICE_ASSISTANT_LANGUAGE_TO_NAME.items()}
    ...
    def _off_peak_charging_changed(
        ...
        off_peak_charging = self.get_property(
            DreameMowerProperty.OFF_PEAK_CHARGING)
        if off_peak_charging and off_peak_charging != "":
            self.status.off_peak_charging_config = json.loads(
                off_peak_charging)
            ...
            if self.status.go_to_zone:
                ...
                try:
                    ...
                    if stop and self.status.started:
                        self._update_status(
                            DreameMowerTaskStatus.COMPLETED, DreameMowerStatus.STANDBY
                        )
                except:
                    pass
    ...
    def connect_device(self) -> None:
        ...
        info = self._protocol.connect(
            self._message_callback, self._connected_callback)
        if info:
                ...
                if self._map_manager:
                    ...
                    if self.status.current_map is None:
                        ...
                        try:
                            ...
                            _LOGGER.error(
                                "Initial map update failed! %s", str(ex))
    ...
    def set_property_value(self, prop: str, value: Any):
        if prop is not None and value is not None:
            ...
            if prop in DreameMowerProperty.__members__:
                ...
                prop = DreameMowerAutoSwitchProperty(
                    DreameMowerAutoSwitchProperty[prop]
                )
            ...
            if prop is DreameMowerProperty.CLEANING_MODE:
                ...
                value = get_int_value(
                    DreameMowerCleaningRoute, value, self.status.cleaning_route_list
                )
    ...
    def get_map_for_render(self, map_data: MapData) -> MapData | None:
        ...
        if map_data:
            ...
            if (
                ...
                render_map_data.segments = copy.deepcopy(
                    saved_map_data.segments)
                render_map_data.data = copy.deepcopy(saved_map_data.data)
                render_map_data.pixel_type = copy.deepcopy(
                    saved_map_data.pixel_type)
                render_map_data.dimensions = copy.deepcopy(
                    saved_map_data.dimensions)
            ...
            offset = render_map_data.dimensions.grid_size / (
                1 if self.capability.map_object_offset else 2
            )
            ...
            if not render_map_data.history_map:
                if self.status.started and not (
                    ...
                    or (
                        ...
                        is DreameMowerTaskStatus.DOCKING_PAUSED
                    )
                ):
    ...
    def update(self, force_request_properties=False) -> None:
        ...
        if (
            ...
            properties.extend(
                [DreameMowerProperty.MAP_LIST, DreameMowerProperty.RECOVERY_MAP_LIST]
            )
            self._last_map_list_request = time.time()
    ...
    def call_stream_audio_action(self, property: DreameMowerProperty, parameters=None):
        return self.call_stream_action(
            DreameMowerAction.STREAM_AUDIO, property, parameters
        )

    def call_stream_video_action(self, property: DreameMowerProperty, parameters=None):
        return self.call_stream_action(
            DreameMowerAction.STREAM_VIDEO, property, parameters
        )

    def call_stream_property_action(
        self, property: DreameMowerProperty, parameters=None
    ):
        return self.call_stream_action(
            DreameMowerAction.STREAM_PROPERTY, property, parameters
        )
    ...
    def call_action(
        self, action: DreameMowerAction, parameters: dict[str, Any] = None
    ) -> dict[str, Any] | None:
        """Call an action."""
        ...
        if action is DreameMowerAction.RESET_BLADES:
            ...
            self._update_property(
                DreameMowerProperty.SIDE_BRUSH_TIME_LEFT, 200)
            ...
            self._update_property(
                DreameMowerProperty.ERROR, DreameMowerErrorCode.NO_ERROR.value
            )
        ...
        self.schedule_update(
            6, bool(not map_action and self._protocol.dreame_cloud))
        if result and result.get("code") == 0:
    ...
    def set_cleaning_mode(self, cleaning_mode: int) -> bool:
        ...
        if self.status.cruising:
            raise InvalidActionException(
                "Cannot set cleaning mode when cruising")
        ...
        if self.status.started and not PROPERTY_AVAILABILITY[
            DreameMowerProperty.CLEANING_MODE.name
        ](self):
    ...
    def set_off_peak_charging_config(self, enabled: bool, start: str, end: str) -> bool:
        ...
        if not re.match(time_pattern, start):
            raise InvalidValueException(
                "Start time is not valid: (%s).", start)
        ...
        return self.set_property(
            DreameMowerProperty.OFF_PEAK_CHARGING,
            str(
                json.dumps(self.status.off_peak_charging_config,
                           separators=(",", ":"))
            ).replace(" ", ""),
        )
    ...
    def set_voice_assistant_language(self, voice_assistant_language: str) -> bool:
        if (
            self.get_property(
                DreameMowerProperty.VOICE_ASSISTANT_LANGUAGE) is None
            or voice_assistant_language is None
            or len(voice_assistant_language) < 2
            or voice_assistant_language.upper()
            not in DreameMowerVoiceAssistantLanguage.__members__
        ):
            raise InvalidActionException(
    ...
    def start_mowing(self) -> dict[str, Any] | None:
        ...
        if not self.status.started:
            ...
            self._update_property(
                DreameMowerProperty.STATUS, DreameMowerStatus.CLEANING.value
            )
            if self.status.task_status is not DreameMowerTaskStatus.COMPLETED:
                new_state = DreameMowerState.MOWING
                self._update_property(
                    DreameMowerProperty.STATE, new_state.value)
    ...
    def start_custom(
        ...
        if (
            not self.capability.cruising
            and status != DreameMowerStatus.ZONE_CLEANING.value
        ):
            self._restore_go_to_zone()

        if (
            status is not DreameMowerStatus.FAST_MAPPING.value
        ...
        if parameters is not None:
            payload.append(
                {
                    "piid": PIID(
                        DreameMowerProperty.CLEANING_PROPERTIES, self.property_mapping
                    ),
                    "value": parameters,
                }
            )
    ...
    def clean_zone(
        ...
        if not self.status.started or self.status.paused:
            self._update_status(
                DreameMowerTaskStatus.ZONE_CLEANING, DreameMowerStatus.ZONE_CLEANING
            )
    ...
    def clean_segment(
        ...
        if self.status.current_map and not self.status.has_saved_map:
            raise InvalidActionException(
                "Cannot clean segments on current map")
    ...
    def clean_spot(
        ...
        if not isinstance(points, list) or not points:
            raise InvalidActionException(
                f"Invalid point coordinates: %s", points)
        ...
        if not self.status.started or self.status.paused:
            self._update_status(
                DreameMowerTaskStatus.SPOT_CLEANING, DreameMowerStatus.SPOT_CLEANING
            )
    ...
    def go_to(self, x, y) -> dict[str, Any] | None:
        ...
        if not (self.status.started or self.status.paused):
            self._update_property(
                DreameMowerProperty.STATE, DreameMowerState.MONITORING.value
            )
            self._update_property(
                DreameMowerProperty.STATUS, DreameMowerStatus.CRUISING_POINT.value
            )
            self._update_property(
                DreameMowerProperty.TASK_STATUS,
    ...
    def follow_path(self, points: list[int] | list[list[int]]) -> dict[str, Any] | None:
        ...
        if not self.capability.cruising:
            raise InvalidActionException(
                "Follow path is supported on this device")
        ...
        if not self.status.started or self.status.paused:
            ...
            self._update_property(
                DreameMowerProperty.STATUS, DreameMowerStatus.CRUISING_PATH.value
            )
            self._update_property(
                DreameMowerProperty.TASK_STATUS,
    ...
    def start_shortcut(self, shortcut_id: int) -> dict[str, Any] | None:
        ...
        if not self.status.started:
            if self.status.status is DreameMowerStatus.STANDBY:
                self._update_property(
                    DreameMowerProperty.STATE, DreameMowerState.IDLE.value
                )

            self._update_property(
                DreameMowerProperty.STATUS, DreameMowerStatus.SEGMENT_CLEANING.value
            )
            self._update_property(
                DreameMowerProperty.TASK_STATUS,
    ...
    def start_fast_mapping(self) -> dict[str, Any] | None:
        ...
        self.schedule_update(10, True)
        self._update_status(
            DreameMowerTaskStatus.FAST_MAPPING, DreameMowerStatus.FAST_MAPPING
        )
    ...
    def remote_control_move_step(
        ...
        if self.status.fast_mapping:
            raise InvalidActionException(
                "Cannot remote control mower while fast mapping"
            )
            ...
            % {
                ...
                "audio": (
                    ...
                    if prompt == True
                        ...
                        or self.status.status is DreameMowerStatus.SLEEPING
                        else "true"
                    )
                ),
        ...
        if self.capability.ai_detection:
            ...
            try:
                if isinstance(ai_value, int):
                    ...
                    result = self.set_ai_detection(
                        {DreameMowerStrAIProperty[prop.name].value: bool(
                            value)}
                    )
    ...
    def set_camera_light_brightness(self, brightness: int) -> dict[str, Any] | None:
        if self.capability.auto_switch_settings:
            ...
            self._update_property(
                DreameMowerProperty.CAMERA_LIGHT_BRIGHTNESS, str(brightness)
            )
            ...
            if result is None or result.get("code") != 0:
                self._update_property(
                    DreameMowerProperty.CAMERA_LIGHT_BRIGHTNESS, str(
                        current_value)
                )
            return result
    ...
    def set_wider_corner_coverage(self, value: int) -> dict[str, Any] | None:
        if self.capability.auto_switch_settings:
            current_value = self.get_auto_switch_property(
                DreameMowerAutoSwitchProperty.WIDER_CORNER_COVERAGE
            )
            if current_value is not None and current_value > 0 and value <= 0:
                value = -current_value
            return self.set_auto_switch_property(
                DreameMowerAutoSwitchProperty.WIDER_CORNER_COVERAGE, value
            )
    ...
    def set_multi_floor_map(self, enabled: bool) -> bool:
        if self.set_property(DreameMowerProperty.MULTI_FLOOR_MAP, int(enabled)):
            if (
                ...
                self.set_property(
                    DreameMowerProperty.INTELLIGENT_RECOGNITION, 0)
            return True
        return False
    ...
    def rename_shortcut(
        ...
        if self.status.started:
            raise InvalidActionException(
                "Cannot rename a shortcut while mower is running"
            )
        ...
        if shortcut_name and len(shortcut_name) > 0:
            ...
            if current_name != shortcut_name:
                ...
                self._update_property(
                    DreameMowerProperty.SHORTCUTS,
                    str(json.dumps(shortcuts, separators=(
                        ",", ":"))).replace(" ", ""),
                )
                self._property_changed()
    ...
    def set_obstacle_ignore(self, x, y, obstacle_ignored) -> dict[str, Any] | None:
        ...
        if not self._map_manager:
            raise InvalidActionException(
                "Obstacle ignore requires cloud connection")

        if self.status.started:
            raise InvalidActionException(
                "Cannot set obstacle ignore status while mower is running"
            )
    ...
    def set_router_position(self, x, y):
        if not self.capability.wifi_map:
            raise InvalidActionException(
                "WiFi map is not available on this device")

        if self.status.started:
            raise InvalidActionException(
                "Cannot set router position while mower is running"
            )
    ...
    def update_map_data_async(self, parameters: dict[str, Any]):
        ...
        parameters = [
            {
                "piid": PIID(
                    DreameMowerProperty.MAP_EXTEND_DATA, self.property_mapping
                ),
        ...
        def callback(result):
            if result and result.get("code") == 0:
                _LOGGER.info(
                    "Send action UPDATE_MAP_DATA async %s", parameters)
                self._last_change = time.time()
            else:
                _LOGGER.error(
    ...
    def set_pathway(self, pathways=[]) -> dict[str, Any] | None:
        ...
        if self._map_manager:
            ...
            if self.status.current_map and not self.status.has_saved_map:
                raise InvalidActionException(
                    "Cannot edit pathways on current map")
    ...
    def set_predefined_points(self, points=[]) -> dict[str, Any] | None:
        ...
        if self.status.started:
            raise InvalidActionException(
                "Cannot set predefined points while mower is running"
            )
        ...
        if self._map_manager:
            ...
            self._map_manager.editor.set_predefined_points(
                predefined_points[:20])
    ...
    def delete_map(self, map_id: int = None) -> dict[str, Any] | None:
        ...
        if self.status.started:
            raise InvalidActionException(
                "Cannot delete a map while mower is running")
    ...
    def restore_map_from_file(
        ...
        if map_recovery_status is None:
            raise InvalidActionException(
                "Map recovery is not supported on this device")
        ...
        if self.status.started:
            raise InvalidActionException(
                "Cannot set restore a map while mower is running"
            )
        ...
        if not response or response[0]["code"] != 0:
            self._update_property(
                DreameMowerProperty.MAP_RECOVERY_STATUS, map_recovery_status
            )
            raise InvalidActionException(
    ...
    def restore_map(
        ...
        if not self._map_manager:
            raise InvalidActionException(
                "Map recovery requires cloud connection")
        ...
        if self.status.has_temporary_map:
            raise InvalidActionException(
                "Restore a map when temporary map is present")
    ...
    def backup_map(self, map_id: int = None) -> dict[str, Any] | None:
        ...
        if not self.capability.backup_map:
            raise InvalidActionException(
                "Map backup is not supported on this device")
    ...
    def set_cleaning_sequence(
        ...
        if self.status.started:
            raise InvalidActionException(
                "Cannot set cleaning sequence while mower is running"
            )
    ...
    def set_custom_cleaning(
        ...
        if self.status.started:
            raise InvalidActionException(
                "Cannot edit customized cleaning parameters while mower is running"
            )
        ...
        if cleaning_times:
            for v in cleaning_times:
                if int(v) < 1 or int(v) > 3:
                    raise InvalidActionException(
                        "Invalid cleaning times: %s", v)

        if cleaning_mode:
            for v in cleaning_mode:
                if int(v) < 0 or int(v) > 2:
                    raise InvalidActionException(
                        "Invalid cleaning mode: %s", v)
    ...
    def set_hidden_segments(self, invisible_segments: list[int]):
        ...
        if self.status.started:
            raise InvalidActionException(
                "Cannot set room visibility while mower is running"
            )
    ...
    def set_segment_cleaning_times(
        ...
        if self.status.started:
            raise InvalidActionException(
                "Cannot set room cleaning times while mower is running"
            )
    ...
    def cloud_connected(self) -> bool:
        """Return connection status of the device."""
        return self._protocol.cloud and self._protocol.cloud.connected
...
class DreameMowerDeviceStatus:
    ...
    def __init__(self, device):
        ...
        self.cleaning_mode_list = {v: k for k,
                                   v in CLEANING_MODE_CODE_TO_NAME.items()}
        self.wider_corner_coverage_list = {
            v: k for k, v in WIDER_CORNER_COVERAGE_TO_NAME.items()
        }
        self.second_cleaning_list = {v: k for k,
                                     v in SECOND_CLEANING_TO_NAME.items()}
        self.cleaning_route_list = {v: k for k,
                                    v in CLEANING_ROUTE_TO_NAME.items()}
    ...
    def charging_status(self) -> DreameMowerChargingStatus:
        ...
        if value is not None and value in DreameMowerChargingStatus._value2member_map_:
            ...
            if (
                value is DreameMowerChargingStatus.CHARGING
    ...
    def relocation_status(self) -> DreameMowerRelocationStatus:
        ...
        if (
            value is not None
            and value in DreameMowerRelocationStatus._value2member_map_
        ):
    ...
    def state(self) -> DreameMowerState:
        ...
        if value is not None and value in DreameMowerState._value2member_map_:
            if self.go_to_zone and (
                value == DreameMowerState.IDLE or value == DreameMowerState.MOWING.value
            ):
                if self.paused:
            ...
            if mower_state is DreameMowerState.IDLE:
                if self.started or self.cleaning_paused or self.fast_mapping_paused:
                    ...
                    if (
                        ...
                        is DreameMowerChargingStatus.CHARGING_COMPLETED
                    ):
    ...
    def wider_corner_coverage(self) -> DreameMowerWiderCornerCoverage:
        ...
        if (
            value is not None
            and value in DreameMowerWiderCornerCoverage._value2member_map_
        ):
    ...
    def wider_corner_coverage_name(self) -> str:
        ...
        if (
            ...
            in DreameMowerWiderCornerCoverage._value2member_map_
        ):
            return WIDER_CORNER_COVERAGE_TO_NAME.get(
                DreameMowerWiderCornerCoverage(
                    wider_corner_coverage), STATE_UNKNOWN
            )
    ...
    def cleaning_route(self) -> DreameMowerCleaningRoute:
        if self._capability.cleaning_route:
            ...
            if (
                value is not None
                and value in DreameMowerCleaningRoute._value2member_map_
            ):
    ...
    def cleaning_route_name(self) -> str:
        ...
        if (
            ...
            and cleaning_route in DreameMowerCleaningRoute._value2member_map_
        ):
            return CLEANING_ROUTE_TO_NAME.get(
                DreameMowerCleaningRoute(cleaning_route), STATE_UNKNOWN
            )
        return STATE_UNKNOWN
    ...
    def cleangenius(self) -> DreameMowerCleanGenius:
        if self._capability.cleangenius:
            value = self._device.get_auto_switch_property(
                DreameMowerAutoSwitchProperty.CLEANGENIUS
            )
            if value is not None and value < 0:
                value = 0
    ...
    def cleangenius_name(self) -> str:
        ...
        if (
            ...
            and cleangenius in DreameMowerCleanGenius._value2member_map_
        ):
            return CLEANGENIUS_TO_NAME.get(
                DreameMowerCleanGenius(cleangenius), STATE_UNKNOWN
            )
        return STATE_UNKNOWN
    ...
    def voice_assistant_language(self) -> DreameMowerVoiceAssistantLanguage:
        ...
        if (
            value is not None
            and value in DreameMowerVoiceAssistantLanguage._value2member_map_
        ):
    ...
    def has_error(self) -> bool:
        ...
        return bool(
            ...
            and error is not DreameMowerErrorCode.BATTERY_LOW
        )
    ...
    def resume_cleaning(self) -> bool:
        ...
        return bool(
            self._get_property(DreameMowerProperty.RESUME_CLEANING)
    ...
    def returning(self) -> bool:
        ...
        return bool(
            self._device_connected and (
                self.status is DreameMowerStatus.BACK_HOME)
        )
    ...
    def max_suction_power(self) -> bool:
        ...
        return bool(
            ...
            and self._get_property(DreameMowerAutoSwitchProperty.MAX_SUCTION_POWER)
        )
                ...
                for history in self._cleaning_history:
                    ...
                    if history.status is not None:
                        list[date][ATTR_STATUS] = (
                            STATUS_CODE_TO_NAME.get(
                                history.status, STATE_UNKNOWN)
                            .replace("_", " ")
                            .capitalize()
                        )
                    ...
                    if history.cleanup_method is not None:
                        list[date][ATTR_CLEANUP_METHOD] = (
                            history.cleanup_method.name.replace(
                                "_", " ").capitalize()
                        )
    ...
    def ai_furniture_detection(self) -> bool:
        return self._device.get_ai_property(
            DreameMowerAIProperty.AI_FURNITURE_DETECTION
        )
    ...
    def fill_light(self) -> bool:
        return self._device.get_auto_switch_property(
            DreameMowerAutoSwitchProperty.FILL_LIGHT
        )
    ...
    def stain_avoidance(self) -> bool:
        return bool(
            self._device.get_auto_switch_property(
                DreameMowerAutoSwitchProperty.STAIN_AVOIDANCE
            )
            == 2
        )
    ...
    def pet_focused_cleaning(self) -> bool:
        return self._device.get_auto_switch_property(
            DreameMowerAutoSwitchProperty.PET_FOCUSED_CLEANING
        )
        ...
        for prop in properties:
            ...
            if value is not None:
                ...
                if prop is DreameMowerProperty.ERROR:
                    ...
                    value = self.cleaning_route_name.replace(
                        "_", " ").capitalize()
                    attributes[f"{prop_name}_list"] = (
                        [
                            v.replace("_", " ").capitalize()
                    ...
                    value = self.cleangenius_name.replace(
                        "_", " ").capitalize()
                    attributes[f"{prop_name}_list"] = (
                        [
                            v.replace("_", " ").capitalize()
        ...
        if self.map_list:
            ...
            if self._capability.lidar_navigation:
                attributes[ATTR_CURRENT_SEGMENT] = (
                    self.current_zone.segment_id if self.current_zone else 0
                )
    ...
    def consumable_life_warning_description(self, consumable_property) -> str:
        description = CONSUMABLE_TO_LIFE_WARNING_DESCRIPTION.get(
            consumable_property)
        if description:
